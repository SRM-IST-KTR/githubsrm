...
def project_single_project(request, **kwargs):
    ...
    entry = AdminEntry()
    try:
        project = entry.db.project.find_one({"_id": request.GET["projectId"]})
        if not project:
        ...
        if request.GET["maintainer"] == "true" and request.GET["contributor"] != "true":
            data = list(
                entry.db.maintainer.find({"project_id": request.GET["projectId"]})
            )
            record["maintainer"] = {"maintainer": data}

        elif (
            request.GET["maintainer"] != "true" and request.GET["contributor"] == "true"
        ):
            data = list(
                entry.db.contributor.find(
...
def accepted_project_pagination(request, **kwargs) -> response.JsonResponse:
    ...
    try:
        ...
        total_docs = open_entry.db.project.count_documents({"is_admin_approved": True})

        records = list(
            open_entry.db.project.aggregate(
                [
        ...
        print(e)
        return response.JsonResponse(
            {"hasNextPage": False, "hasPreviousPage": False, "record": []}, status=200
        )


def alpha_maintainer_support(
    existing: bool, project: Dict[str, str], maintainer: Dict[str, str], request
) -> Dict[str, str]:
    ...
    if existing:
        if service.wrapper_email(
            role="project_submission_approval",
            data={
            ...
            return {"message": {"error": "email failed"}, "status": 500}
    else:
        password = RequestSetPassword(email=request.data.get("email"))
        if service.wrapper_email(
            role="project_submission_approval_w_password_link",
            data={
                "name": maintainer["name"],
                "project_name": project["project_name"],
            ...
            return {"message": {"success": "Approved new maintainer"}, "status": 200}
...
def beta_maintainer_support(
    ...
    if existing:
        ...
        if service.wrapper_email(
            role="welcome_maintainer_w_password_link",
            data={
                "name": maintainer["name"],
                "project_name": project["project_name"],
                "reset_token": password,
